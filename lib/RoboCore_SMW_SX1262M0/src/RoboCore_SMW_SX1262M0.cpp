...
// Get the Confirm Mode
//  @param (adr) : the variable to store the result [uint8_t (&)]
//  @returns the type of the response [CommandResponse]
CommandResponse SMW_SX1262M0::get_CFM(uint8_t (&cfm)){
  // send the command and read the response
  _send_command(CMD_CFM, CommandAction::GET);
  CommandResponse res = _read_response(SMW_SX1262M0_TIMEOUT_READ);
...
//  @param (status) : the variable to store the result [uint8_t (&)]
//  @returns the type of the response [CommandResponse]
CommandResponse SMW_SX1262M0::get_ConfStatus(uint8_t (&status)){
  // send the command and read the response
  _send_command(CMD_CFS, CommandAction::GET);
  CommandResponse res = _read_response(SMW_SX1262M0_TIMEOUT_READ);
...
//      _connected = (b == '1') ? true : false; // update (ASCII of SMW_SX1262M0_TX_STATUS_CONFIRMED)
    }
  }
...
//  @returns true if the last send received a confirmation from network [bool]
//  NOTE: it is a wrapper around the <get_ConfStatus()> command
bool SMW_SX1262M0::isConfirmed(void){
  uint8_t status = SMW_SX1262M0_TX_STATUS_NOT_CONFIRMED; // default
  if(get_ConfStatus(status) == CommandResponse::OK){
    if(status == SMW_SX1262M0_TX_STATUS_CONFIRMED){
      return true;
    }
  }
  return false; // default
...
// Set the Confirm Mode
//  @param (cfm) : the data to be sent [uint8_t]
//  @returns the type of the response [CommandResponse]
CommandResponse SMW_SX1262M0::set_CFM(uint8_t cfm){
  cfm = (cfm == SMW_SX1262M0_CFM_ON) ? SMW_SX1262M0_CFM_ON : SMW_SX1262M0_CFM_OFF; // force binary value
  cfm += '0'; // convert to ASCII character, without a narrowing conversion
  unsigned char data[] = { cfm , CHAR_EOS};
  
  // send the command and read the response
  _send_command(CMD_CFM, CommandAction::SET, 1, data);
  CommandResponse res = _read_response(SMW_SX1262M0_TIMEOUT_WRITE); // this command takes almost X s to reply
